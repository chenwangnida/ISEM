...
import de.dfki.s2m2.MatchingSetup;
...
public class ISeMFactory {
	...
	protected static Logger		logger = Logger.getLogger(ISeMFactory.class);
	...
	public static MatchingEngine createMatchingEngine(URI filterSetupUri) {
		...
		// iSeM
		logger.debug("Initialize iSeM filter packages.");
		de.dfki.isem.s2m2.filter.FilterPackage iSeMPackage = de.dfki.isem.s2m2.filter.FilterPackage.eINSTANCE; 
		de.dfki.isem.s2m2.filter.learning.LearningPackage learningPackage = de.dfki.isem.s2m2.filter.learning.LearningPackage.eINSTANCE;
		de.dfki.isem.s2m2.filter.learning.libsvm.LibsvmPackage libsvmPackage = de.dfki.isem.s2m2.filter.learning.libsvm.LibsvmPackage.eINSTANCE;
		...
		for(de.dfki.s2m2.filter.Resource resource : config.getResource()) {
			try {
				...
				// register service registry and view extractor for iSeMReasoner instances
				try {
					iSeMReasoner isem = (iSeMReasoner) reasoner;
					isem.setServiceRegistry(registry);
					isem.setViewExtractor(hybridViewExtractor);
				}
	...
	public static void main(String[] args) {
		createMatchingEngine(URI.createFileURI("C:\\Daten\\workspaces\\runtime-EclipseApplication\\iSeM filters\\approx-logic-based.filter"));
	}
}
